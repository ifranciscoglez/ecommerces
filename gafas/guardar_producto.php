<html>
    ...
    <body>
        ...
        if(!$codigo=="" && !$nombre=="" && !$descripcion=="" && $extractoDes=="" && $color=="" && !$precio=="" && !$stock=="" && !$categoria=="" && !$_FILES['img']['name']==""){
            $verificar=  ejecutarSQL::consultar("select * from gafas where CodGafas='".$codigo."'");
            $verificaltotal = mysql_num_rows($verificar);
            if($verificaltotal<=0){
                if(move_uploaded_file($_FILES['img']['tmp_name'],"../assets/img-products/".$_FILES['img']['name'])){
                    if(consultasSQL::InsertSQL('codGafas', 'nombreGafas', 'descGafas', 'precioGafas', 'stockGafas', 'imgGafas', 'Categoria_idCategoria', "'$codeProd','$nameProd','$cateProd','$priceProd', '$modelProd','$marcaProd','$stockProd','$codePProd','".$_FILES['img']['name']."','$adminProd'")){
                       echo '
